...
def get_data(payload_type):
    if payload_type == eb.PvPayloadTypeImage:
        ...
        print(
            f" Chunk Data payload type with {pvbuffer.GetChunkCount()} chunks", end=""
        )

    elif payload_type == eb.PvPayloadTypeRawData:
        print(
            f" Raw Data with {pvbuffer.GetRawData().GetPayloadLength()} bytes", end=""
        )
            ...
            if result.IsOK():
                ...
                if result.IsOK():
                    ...
                    if calculated_size != decompressed_size:
                        errors = errors + 1
                    print(
                        f" Pleora compressed type.   Compression ratio: {'{0:.2f}'.format(compression_ratio)} Errors: {errors}",
                        end="",
                    )
                else:
...
def crear_patron(resolucion, orientacion, mitad, intensidad):
    ...
    if orientacion == "horizontal":
        ...
        if mitad == "sup":
            grayscale_array[:half_height, :] = (
                intensidad  # llena las filas hasta half_height con intensidad
            )
        elif mitad == "inf":
            grayscale_array[half_height:, :] = (
                intensidad  # llena las filas desde half_height con intensidad
            )
...
intensidades_array = np.arange(0, 256, 1)
...
T_dia = 21
cant_promedio = 10
save_dir = r"data\seleccion_de_tensiones\24-c4 0.69-1.42\ims_gamma_lineal_pasos5"
...
for i in intensidades_array:
    ...
    err, dataHandle = slm.loadImageData(
        patron
    )  # carga la data (array) a la video memory del display SLM
    assert err == HEDSERR_NoError, HEDS.SDK.ErrorString(err)
    ...
    if cant_promedio == 1:
        ...
        for h in range(cant_promedio):
            cv2.imwrite(f"{file_name}_{h}.png", lista_promedio[h])
