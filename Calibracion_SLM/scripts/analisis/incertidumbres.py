...
imagen = simular_imagen(frecuencia=100, angulo_franjas_max=0, angulo_slm_max=0, fase2 = np.pi)
...
if prueba:
    ...
    for i in range(256):
        imagen = simular_imagen(frecuencia=100, angulo_franjas_max=0, angulo_slm_max=0, fase2 = fases[i], amplitud_imperfecciones=0.1)
        #plt.imshow(imagen)
        #plt.pause(.01)
        #plt.cla()
        recorte1 = imagen[y_rec1:y_rec1+altura_rec, x_rec1:x_rec1+ancho_rec]
        recorte2 = imagen[y_rec2:y_rec2+altura_rec, x_rec2:x_rec2+ancho_rec]
        c1[i], c2[i] = fase_hilbert(recorte1, recorte2, filter_frec = 0.15666666666666668)
        print(i)
    grises = np.arange(256)
    exp_1 = np.exp(1j*c1)
    exp_2 = np.exp(1j*c2)
    dif_compleja = np.angle(exp_1/exp_2)
    plt.scatter(grises, dif_compleja)
    plt.show()
    calibracion = np.unwrap(dif_compleja)
    plt.scatter(grises, calibracion)
    plt.show()
    diferencia = calibracion - fases
    plt.scatter(grises, diferencia)
    plt.show()